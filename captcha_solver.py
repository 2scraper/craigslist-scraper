...
page. If Craigslist renders a reCAPTCHA challenge anywhere — the posting
form and account flows are the usual places — this fires.

**NO challenge of any kind has been observed on the pages this scraper
reads.** Measured 2026-09-14 across 22 captures covering six categories and
five locales: zero occurrences of reCAPTCHA, hCaptcha, Turnstile, DataDome,
PerimeterX, Incapsula, Akamai or AWS WAF markers, no challenge iframe, no
`data-sitekey` — and the word "captcha" itself appears zero times on any of
them. So this module is a contingency, not part of the happy path: a bot
manager can be switched on between deploys, and a scraper that cannot name
what stopped it is much harder to fix than one that can.

What Craigslist does to an address it has scored has NOT been measured here
— no run during this repo's development was refused — so this file does not
claim to know. `product_parser.detect_page_state` answers that question
structurally instead, by asking whether a response was built out of the
site's own assets, which is correct for an interstitial, a block page and
Chromium's own network-error page alike.
...
# of it is ported here: Craigslist serves no such page to a visitor reading
# result lists, and porting a solver for a challenge that has never been
# observed would be dead code that reads as enforcement.
#
# What Craigslist serves a refused address is not known from measurement here
# — nothing this repo ran was refused — so nothing in this file guesses at
# it. detect_page_state decides that structurally rather than by marker.
#
